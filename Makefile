...
# Vulkan SDK - set VULKAN_SDK to your SDK root
# Windows example: make VULKAN_SDK=D:/code/vulkan/1.4.341.1
# Linux:           pkg-config auto-detects, or set VULKAN_SDK
ifdef VULKAN_SDK
SDL += libs/sdl/vulkan.o
VULKAN_CPPFLAGS = -I$(VULKAN_SDK)/Include
VULKAN_LDFLAGS = -L$(VULKAN_SDK)/Lib
VULKAN_LDLIBS = -lvulkan-1
libs/sdl/vulkan.o: CPPFLAGS += $(VULKAN_CPPFLAGS) -DHL_VULKAN
sdl_LDFLAGS += $(VULKAN_LDFLAGS)
sdl_LDLIBS += $(VULKAN_LDLIBS)
else ifneq ($(OS),Windows_NT)
ifneq ($(UNAME),Darwin)
SDL += libs/sdl/vulkan.o
VULKAN_CPPFLAGS = $(shell pkg-config --cflags vulkan 2>/dev/null || echo "")
