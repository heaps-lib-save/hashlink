...
HL_PRIM void HL_NAME(set_vsync)(bool v) {
	int interval = v ? 1 : 0;
#ifdef _WIN32
	// Bypass SDL: call wglSwapIntervalEXT directly via opengl32.dll
	// SDL_GL_SetSwapInterval is unreliable on some drivers (notably NVIDIA).
	HGLRC ctx = wglGetCurrentContext();
	if (ctx) {
		typedef BOOL (WINAPI *WglSwapIntervalProc)(int);
		typedef PROC (WINAPI *WglGetProcAddressProc)(LPCSTR);
		static WglSwapIntervalProc wglSwapInterval = NULL;
		if (!wglSwapInterval) {
			HMODULE gl = GetModuleHandleA("opengl32.dll");
			if (gl) {
				WglGetProcAddressProc wglGPA = (WglGetProcAddressProc)GetProcAddress(gl, "wglGetProcAddress");
				if (wglGPA)
					wglSwapInterval = (WglSwapIntervalProc)wglGPA("wglSwapIntervalEXT");
			}
		}
		if (wglSwapInterval) {
...
// ============ Frame Pacer ============
#if defined(_WIN32)
// Windows: QPC-based, sub-microsecond precision
static LARGE_INTEGER g_pacerFreq;
static double         g_pacerTargetFps;
static LARGE_INTEGER  g_pacerNextFrame;
static LARGE_INTEGER  g_pacerEntryTime;
static LARGE_INTEGER  g_pacerPrevEntryTime;

HL_PRIM void HL_NAME(pacer_init)(double target_fps) {
	QueryPerformanceFrequency(&g_pacerFreq);
	g_pacerTargetFps = target_fps > 0 ? target_fps : 0;
	QueryPerformanceCounter(&g_pacerNextFrame);
	g_pacerEntryTime = g_pacerNextFrame;
	g_pacerPrevEntryTime = g_pacerNextFrame;
}

HL_PRIM void HL_NAME(pacer_mark)() {}

HL_PRIM void HL_NAME(pacer_wait)() {
	if (g_pacerTargetFps <= 0 || g_pacerFreq.QuadPart == 0) return;

	LONGLONG targetTicks = (LONGLONG)(g_pacerFreq.QuadPart / g_pacerTargetFps);
	LARGE_INTEGER now;
	QueryPerformanceCounter(&now);

	if (now.QuadPart >= g_pacerNextFrame.QuadPart) {
		g_pacerNextFrame.QuadPart = now.QuadPart + targetTicks;
		g_pacerPrevEntryTime = g_pacerEntryTime;
		QueryPerformanceCounter(&g_pacerEntryTime);
		return;
	}

	LONGLONG remainTicks = g_pacerNextFrame.QuadPart - now.QuadPart;
	if (remainTicks > g_pacerFreq.QuadPart / 100) {
		DWORD sleepMs = (DWORD)((remainTicks * 1000) / g_pacerFreq.QuadPart) - 1;
		if (sleepMs > 0) Sleep(sleepMs);
	}

	while (true) {
		QueryPerformanceCounter(&now);
		if (now.QuadPart >= g_pacerNextFrame.QuadPart) break;
		YieldProcessor();
	}

	g_pacerNextFrame.QuadPart += targetTicks;
	g_pacerPrevEntryTime = g_pacerEntryTime;
	g_pacerEntryTime = now;
}

HL_PRIM double HL_NAME(pacer_interval)() {
	if (g_pacerFreq.QuadPart == 0) return 0.0;
	return (double)(g_pacerEntryTime.QuadPart - g_pacerPrevEntryTime.QuadPart) / (double)g_pacerFreq.QuadPart;
}

#else
// Non-Windows: clock_gettime-based (Linux, Android, macOS)
#include <time.h>

static int64_t g_pacerIntervalNs;
static int64_t g_pacerNextFrameNs;
static int64_t g_pacerEntryNs;
static int64_t g_pacerPrevEntryNs;

static int64_t pacer_now_ns() {
	struct timespec ts;
	clock_gettime(CLOCK_MONOTONIC, &ts);
	return (int64_t)ts.tv_sec * 1000000000LL + ts.tv_nsec;
}

static void pacer_yield() {
#if defined(__aarch64__) || defined(__arm__)
	__asm__ __volatile__("yield");
#elif defined(__x86_64__) || defined(__i386__)
	__asm__ __volatile__("pause");
#else
	// fallback: brief sleep to yield CPU
...
HL_PRIM void HL_NAME(pacer_init)(double target_fps) {
	g_pacerIntervalNs = (target_fps > 0) ? (int64_t)(1000000000.0 / target_fps) : 0;
	g_pacerNextFrameNs = pacer_now_ns();
	g_pacerEntryNs = g_pacerNextFrameNs;
	g_pacerPrevEntryNs = g_pacerNextFrameNs;
}

HL_PRIM void HL_NAME(pacer_mark)() {}

HL_PRIM void HL_NAME(pacer_wait)() {
	if (g_pacerIntervalNs <= 0) return;

	int64_t now = pacer_now_ns();

	if (now >= g_pacerNextFrameNs) {
		g_pacerNextFrameNs = now + g_pacerIntervalNs;
		g_pacerPrevEntryNs = g_pacerEntryNs;
		g_pacerEntryNs = now;
		return;
	}

	int64_t remain = g_pacerNextFrameNs - now;
	if (remain > 2000000LL) {
		struct timespec req = {0, remain - 1000000LL};
		nanosleep(&req, NULL);
	}

	while (1) {
		now = pacer_now_ns();
		if (now >= g_pacerNextFrameNs) break;
		pacer_yield();
	}

	g_pacerNextFrameNs += g_pacerIntervalNs;
	g_pacerPrevEntryNs = g_pacerEntryNs;
	g_pacerEntryNs = now;
}

HL_PRIM double HL_NAME(pacer_interval)() {
	if (g_pacerIntervalNs <= 0) return 0.0;
	return (double)(g_pacerEntryNs - g_pacerPrevEntryNs) / 1000000000.0;
}
#endif
...
DEFINE_PRIM(_VOID, pacer_init, _F64);
DEFINE_PRIM(_VOID, pacer_mark, _NO_ARG);
DEFINE_PRIM(_VOID, pacer_wait, _NO_ARG);
DEFINE_PRIM(_F64, pacer_interval, _NO_ARG);
...
HL_PRIM void HL_NAME(win_destroy_vk)(SDL_Window *win) {
	SDL_DestroyWindow(win);
}
...
DEFINE_PRIM(_VOID, win_destroy_vk, TWIN);
...
// writable path for shader cache etc.
static char g_writable_path[512] = {0};
HL_PRIM vbyte *HL_NAME(get_writable_path)() {
	if (g_writable_path[0] == 0) {
		#if defined(__ANDROID__)
		const char *p = SDL_GetAndroidInternalStoragePath();
		#else
		const char *p = ".";
		#endif
		SDL_strlcpy(g_writable_path, p, sizeof(g_writable_path));
	}
	return (vbyte*)g_writable_path;
}
...
DEFINE_PRIM(_BYTES, get_writable_path, _NO_ARG);

#ifdef _WIN32
static void beap_init_dll_search_path(void) {
	WCHAR exePath[MAX_PATH];
	DWORD len = GetModuleFileNameW(NULL, exePath, MAX_PATH);
	if (len > 0 && len < MAX_PATH) {
		WCHAR *slash = wcsrchr(exePath, L'\\');
		if (slash) {
			wcscpy(slash + 1, L"lib");
			SetDllDirectoryW(exePath);
		}
	}
}
#pragma section(".CRT$XCU", read)
__declspec(allocate(".CRT$XCU")) void (*_beap_dll_init)(void) = beap_init_dll_search_path;
#pragma comment(linker, "/include:_beap_dll_init")
#endif
